...
def generate_extraction_query(
    limit: int = None, year: int = None, recent_days: int = None
):
    """
    ...
    if year:
        query += (
            f" AND EXTRACT(YEAR FROM PARSE_DATE('%Y%m%d', "
            f"CAST(filing_date AS STRING))) = {year}"
        )

    if recent_days:
        query += (
            f" AND PARSE_DATE('%Y%m%d', CAST(filing_date AS STRING)) >= "
            f"DATE_SUB(CURRENT_DATE(), INTERVAL {recent_days} DAY)"
        )
...
def generate_create_table_sql(table_name: str = 'patents'):
    ...
    CREATE INDEX idx_{table_name}_inventor_names ON {table_name} USING GIN(
        inventor_names
    );
    CREATE INDEX idx_{table_name}_assignee_names ON {table_name} USING GIN(
        assignee_names
    );
    """
