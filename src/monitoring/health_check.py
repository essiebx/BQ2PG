    ...
    def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            result = HealthCheckResult(
                ...
                timestamp=datetime.now(),
                message="Database connection OK" if latency < 1000 else "Database connection slow",
    ...
    def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            result = HealthCheckResult(
                ...
                timestamp=datetime.now(),
                message="BigQuery connection OK" if latency < 2000 else "BigQuery connection slow",
                checks={"latency_ms": latency},
                latency_ms=latency
            )
...
class PipelineHealthChecker(HealthChecker):
    ...
    def __init__(self, max_failed_records: int = 100, max_dlq_entries: int = 50):
    ...
    def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            if health_checks["failed_records"] > self.max_failed_records:
                status = HealthStatus.UNHEALTHY
                message = f"Too many failed records: {health_checks['failed_records']}"
            ...
            metrics.set_custom_metric("pipeline_health_status", 1 if status == HealthStatus.HEALTHY else 0)
            return result
