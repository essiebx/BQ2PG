...
tracer = Tracer(service_name="bq2pg_transformer")
...
class DataTransformer:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            Cleaned DataFrame
        """
        with tracer.trace_span("clean_data", {"rows": len(df), "columns": len(df.columns)}):
            try:
                structured_logger.info(f"Starting data cleaning")
                ...
                for col in df_clean.columns:
                    if df_clean[col].isna().any():
                        ...
                        structured_logger.info(
                            ...
                            strategy="median" if df_clean[col].dtype in ['int64', 'float64'] else "empty"
                        )
                ...
                metrics.set_custom_metric("cleaning_rows_removed", duplicates_removed)
    ...
    @timer
    def validate_data(self, df: pd.DataFrame, rules: RuleSet = None) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if rules:
                    for i, row in df.iterrows():
                        ...
                        if rule_violations:
                            validation_results[f"row_{i}_violations"] = rule_violations
                ...
                quality_score = (valid_rows / len(df)) * 100 if len(df) > 0 else 100
                ...
                metrics.set_custom_metric("validation_quality_score", quality_score)
    ...
    @timer
    def transform(self, df: pd.DataFrame, transformations: List[str] = None) -> pd.DataFrame:
        """
        ...
        """
        with tracer.trace_span("transform", {"rows": len(df), "transformations": len(transformations or [])}):
            try:
                df_transformed = df.copy()
                transformations = transformations or list(self.transformations.keys())
                ...
                for transform_name in transformations:
                    ...
                    with tracer.trace_span(f"apply_transformation_{transform_name}", {"rows": len(df_transformed)}):
                        try:
                            ...
                            metrics.increment_custom_metric(f"transformations_applied_{transform_name}")
                            ...
                            metrics.increment_custom_metric(f"transformation_failures_{transform_name}")
                ...
                checkpoint_mgr.save_checkpoint(
                    "transformation",
                    {"rows_transformed": len(df_transformed), "transformations": transformations},
        ...
        with tracer.trace_span("process_pipeline", {"rows": len(df)}):
            try:
                ...
                if not memory_optimizer.check_memory_usage():
                    structured_logger.warning("Memory usage high, optimizing...")
                ...
                if validate:
                    with tracer.trace_span("pipeline_validate"):
                        validation_report = self.validate_data(df_processed, rules)
                ...
                if transformations or self.transformations:
                    with tracer.trace_span("pipeline_transform"):
                        df_processed = self.transform(df_processed, transformations)
                ...
                metrics.set_custom_metric("pipeline_rows_processed", result["final_rows"])
...
def deduplicate_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key_cols = [col for col in df.columns if col not in ['created_at', 'updated_at']]
